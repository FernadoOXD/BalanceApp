...
<div class="nutricore-layout">
  ...
  async loadSpecialistSettings() {
    try {
      // Conectar con la API para obtener los settings del especialista
      // const response = await fetch('http://localhost:8080/api/especialista/settings');
      // this.specialistSettings = await response.json();

      // MOCK DATA: Simula los datos del especialista (mismo formato que SettingEspecialista.js)
      this.specialistSettings = {
        duration: 60,
        notifications: true,
        schedule: [
          {
          ...
          // Domingo no está en la lista, por lo que no estará activo
        ],
      };
  ...
  _renderTimeSlots(dayOfWeek) {
    ...
    if (!schedule || !schedule.active) {
      timesGrid.innerHTML = "<p class='n-times__empty'>No hay horarios disponibles para este día.</p>";
      return;
    }

    const duration = this.specialistSettings?.duration || 60;
    const timeSlots = this._generateTimeSlots(schedule.start, schedule.end, duration);
  ...
  async _handleSubmit(e) {
    ...
    // Estructura simplificada y adaptada para Java Backend
    const finalPayload = {
      cita: {
        fecha: formatDateToMySQL(this.state.selectedDate), // Ej. "2026-10-15"
        hora: this.state.selectedTime, // Ej. "10:00 AM"
        motivoConsulta: surveyData.motivoConsulta,
      },
      encuesta: surveyData,
    };

    console.log("Payload listo para el backend Java:", finalPayload);
    ...
    try {
      // Llamada al puerto 8080 (Convención típica de Java Spring Boot / Javalin)
      const response = await fetch("http://localhost:8080/api/citas/agendar", {
        method: "POST",
        headers: {
