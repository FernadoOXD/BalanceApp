...
export class ListPacientesEspecialistaPage extends HTMLElement {
  constructor() {
    ...
    this.citaProgramadaActual = {
      ...
      notasPreparatorias:
        "Verificar adherencia al plan actual y discutir síntomas de inflamación mencionados en la consulta anterior.",
    };
    ...
    this.pacientes = [
      ...
      },
      {
        ...
        iniciales: "LS",
        avatar: "assets/avatars/laura.png",
        ultimoReporte: "Hoy, 09:30 AM",
        estado: "Baja (45%)",
        esNuevo: false,
      },
      {
        ...
        iniciales: "LK",
        avatar: "assets/avatars/leon.png",
        ultimoReporte: "Ayer",
        estado: "Media (78%)",
        esNuevo: true,
      },
      {
        ...
        iniciales: "RS",
        avatar: "assets/avatars/roberto.png",
        ultimoReporte: "Ayer",
        estado: "Media (78%)",
        esNuevo: false,
      },
    ...
    this.expedientes = {
      "PAC-1043": {
        ...
        objetivo:
          "Aplicar la metodología adecuada para realizar diagnósticos del estado nutricio en la juventud...",
        altura: 1.48,
        peso: 64.5,
        talla: 1.48,
        imc: 22.8,
        ...
        frecuenciaAlimentos: [
          {
            ...
            frecuencia: "Diario, 2 veces al día",
            observaciones: "Leche entera",
          },
          {
            grupoAlimento: "Leguminosas",
            frecuencia: "Diario, 1 o 2 veces al día",
            observaciones: "Frijoles refritos",
          },
        ],
        recordatorio24h: [
          {
            comida: "Desayuno",
            lunes: "Huevo (2 piezas) con chorizo (20 g)",
            martes: "",
            miercoles: "",
            jueves: "",
            viernes: "",
        ...
        diagnosticoGeneral:
          "Se ajustan los macronutrientes para la Fase 2 del plan desinflamatorio.",
        observaciones:
          "• Paciente refiere sentirse muy bien, con menor inflamación abdominal.\n• Hubo algunos deslices el fin de semana por un evento social, pero retomó el plan con facilidad.",
        clinicos: {
          alcohol: "No",
          tabaco: "No",
          habitosToxicos: "• Ninguno reportado",
          patologias: "Ninguna",
          gastritis: "No",
          colitis: "No",
          estrenimiento: "No",
          ...
          antecedentesFamiliares:
            "• Sin antecedentes familiares relevantes reportados",
        },
        antropometria: {
          biceps: 12,
          triceps: 12,
  ...
  cambiarVista(nuevaVista, idPaciente = null) {
    ...
    if (idPaciente) {
      this.pacienteSeleccionado =
        this.pacientes.find((p) => p.id === idPaciente) || this.pacientes[2];
    }
  ...
  filtrarHistorialLive(query = "") {
    ...
    const historialesFiltrados = this.historialExpedientes
      ...
      .filter((entry) => {
        ...
        const fechaFormateada = this.formatearFechaEs(
          entry.datos?.fecha,
        ).toLowerCase();
        const fechaPartes = fechaOrig.split("-").reverse().join("/");
        const nombre =
          `${entry.datos?.nombrePaciente || ""} ${entry.datos?.apellidoPaterno || ""} ${entry.datos?.apellidoMaterno || ""}`.toLowerCase();
        return (
          fechaOrig.includes(termino) ||
          fechaFormateada.includes(termino) ||
          fechaPartes.includes(termino) ||
          nombre.includes(termino)
        );
      });

    container.innerHTML = this.renderListaTarjetasHistorial(
  ...
  renderTablasDinamicasFormulario() {
    ...
    if (theadFrecuencias && bodyFrecuencias) {
      ...
      `;
      bodyFrecuencias.innerHTML = this.tablaFrecuenciaTemporal
        .map(
          (row, rowIdx) => `
    ...
    if (theadRegistro && bodyRegistro24h) {
      ...
      `;
      bodyRegistro24h.innerHTML = this.tablaRegistro24hTemporal
        .map(
          (row, rowIdx) => `
    ...
    setTimeout(() => {
      this.querySelectorAll(
        ".input-header-frecuencia, .input-header-registro24h, .input-celda-frecuencia, .input-celda-registro24h",
      ).forEach((textarea) => {
        textarea.style.height = "auto";
        textarea.style.height = `${textarea.scrollHeight}px`;
      });
  ...
  setupMenuContextualTablas() {
    ...
    celdasFrecuencia.forEach((cell) => {
      cell.addEventListener("contextmenu", (e) => {
        e.preventDefault();
        e.stopPropagation();
        const rowIndex = cell.hasAttribute("data-row-index")
          ? parseInt(cell.getAttribute("data-row-index"), 10)
          : -1;
        const colIndex = parseInt(cell.getAttribute("data-col-index"), 10);
        this.mostrarMenuContextualPersonalizado(
          e.clientX,
          e.clientY,
          "frecuencia",
          rowIndex,
  ...
  mostrarMenuContextualPersonalizado(x, y, tipoTabla, rowIndex, colIndex) {
    ...
    itemAnadirFila.addEventListener("click", () => {
      const insertIdx =
        rowIndex !== -1
          ? rowIndex + 1
          : tipoTabla === "frecuencia"
            ? this.tablaFrecuenciaTemporal.length
            : this.tablaRegistro24hTemporal.length;
      if (tipoTabla === "frecuencia") {
        const newRow = {};
        this.columnasFrecuenciaTemporal.forEach((col, idx) => {
          newRow[col.id] = idx === 0 ? "Nuevo Grupo" : "";
        });
        this.tablaFrecuenciaTemporal.splice(insertIdx, 0, newRow);
      } else {
        const newRow = {};
        this.columnasRegistro24hTemporal.forEach((col, idx) => {
          newRow[col.id] = idx === 0 ? "Nueva Comida" : "";
        });
    ...
    itemAnadirColumna.addEventListener("click", () => {
      ...
      if (tipoTabla === "frecuencia") {
        ...
        this.tablaFrecuenciaTemporal.forEach((row) => {
          row[newColId] = "";
        });
      } else {
        this.columnasRegistro24hTemporal.splice(colIndex + 1, 0, newColObj);
        this.tablaRegistro24hTemporal.forEach((row) => {
          row[newColId] = "";
        });
      }
      this.renderTablasDinamicasFormulario();
  ...
  setupHistorialCardClicks() {
    this.querySelectorAll(".tarjeta-historial-click").forEach((tarjeta) => {
      tarjeta.addEventListener("click", (e) => {
        ...
        const coincidencia = this.historialExpedientes.find(
          (entry) =>
            entry.pacienteId === pId && entry.datos.fecha === fechaSeleccionada,
        );

        if (coincidencia) {
          this.expedientes[pId] = JSON.parse(
            JSON.stringify(coincidencia.datos),
          );
  ...
  setupEventListeners() {
    ...
    if (footerPacientes) {
      footerPacientes.addEventListener("click", () => {
        this.limitePacientes =
          this.limitePacientes > 3 ? 3 : this.pacientes.length;
        this.render();
      });
    }
    ...
    this.querySelectorAll(".btn-ver-resumen-cita").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        const citaDetalles = await this.obtenerDetallesCitaBackend(
          citaId,
          pacienteId,
          fechaIso,
        );
    ...
    if (selectAccionCita) {
      selectAccionCita.addEventListener("change", (e) => {
        ...
        if (accion === "cancelar") {
          if (
            confirm(
              "¿Estás seguro de que quieres CANCELAR esta cita programada?",
            )
          ) {
    ...
    const btnVerDetallesComp = this.querySelector(
      "#btn-ver-detalles-completos",
    );
    ...
    if (btnCancelarFormulario) {
      btnCancelarFormulario.addEventListener("click", () => {
        const destino = this.expedientes[this.pacienteSeleccionado?.id]
          ? "ver-expediente"
          : "perfil";
    ...
    const btnEliminarExpediente = this.querySelector(
      "#btn-eliminar-expediente",
    );
    if (btnEliminarExpediente) {
      btnEliminarExpediente.addEventListener("click", () => {
        if (
          confirm(
            "¿Estás seguro de que quieres borrar este expediente? No se puede recuperar.",
          )
        ) {
    ...
    const btnBuscarHist = this.querySelector(
      "#btn-buscar-expediente-historico",
    );
    if (inputBuscarHist) {
      ...
      formExpediente.addEventListener("submit", (e) => {
        ...
        const nuevoExpedienteStruct = {
          ...
          clinicos: {
            ...
            antecedentesFamiliares: this.querySelector(
              "#form-antecedentesFamiliares",
            ).value,
          },
          columnasFrecuencia: JSON.parse(
            JSON.stringify(this.columnasFrecuenciaTemporal),
          ),
          frecuenciaAlimentos: JSON.parse(
            JSON.stringify(this.tablaFrecuenciaTemporal),
          ),
          columnasRegistro24h: JSON.parse(
            JSON.stringify(this.columnasRegistro24hTemporal),
          ),
          recordatorio24h: JSON.parse(
            JSON.stringify(this.tablaRegistro24hTemporal),
          ),
          observaciones: this.querySelector("#form-observaciones").value,
          diagnosticoGeneral: this.querySelector("#form-diagnosticoGeneral")
            .value,
          antropometria: {
            biceps: parseFloat(this.querySelector("#form-biceps").value) || 0,
            triceps: parseFloat(this.querySelector("#form-triceps").value) || 0,
            suprailiaco:
              parseFloat(this.querySelector("#form-suprailiaco").value) || 0,
            musloPliegue:
              parseFloat(this.querySelector("#form-musloPliegue").value) || 0,
            piernaPliegue:
              parseFloat(this.querySelector("#form-piernaPliegue").value) || 0,
            brazoContraido:
              parseFloat(this.querySelector("#form-brazoContraido").value) || 0,
            brazoRelajado:
              parseFloat(this.querySelector("#form-brazoRelajado").value) || 0,
            antebrazo:
              parseFloat(this.querySelector("#form-antebrazo").value) || 0,
            muneca: parseFloat(this.querySelector("#form-muneca").value) || 0,
            cadera: parseFloat(this.querySelector("#form-cadera").value) || 0,
            musloCinta:
              parseFloat(this.querySelector("#form-musloCinta").value) || 0,
            pantorrilla:
              parseFloat(this.querySelector("#form-pantorrilla").value) || 0,
        ...
        this.expedientes[pId] = JSON.parse(
          JSON.stringify(nuevoExpedienteStruct),
        );

        const nombreCompleto =
          `${nuevoExpedienteStruct.nombrePaciente} ${nuevoExpedienteStruct.apellidoPaterno} ${nuevoExpedienteStruct.apellidoMaterno}`.trim();
        this.historialExpedientes.push({
      ...
      const filtrarYMostrarResultados = () => {
        ...
        if (filtrados.length === 0) {
          resultsList.innerHTML =
            '<div class="dropdown-empty-state">No hay coincidencias</div>';
        } else {
